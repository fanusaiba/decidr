  ...
  try {
    ...
    decisions.forEach((d) => {
      ...
      if (!categoryMap[d.category]) {
        categoryMap[d.category] = { totalDelta: 0, count: 0 };
      }
    ...
    const categoryBreakdown = Object.entries(categoryMap).map(
      ([category, data]) => ({
        category,
        avgImpact: parseFloat(
          (data.totalDelta / data.count).toFixed(1)
        ),
        count: data.count,
      })
    );
    ...
    res.json({
      totalDecisions,
      avgProductivityGain: parseFloat(
        (totalProd / safeCount).toFixed(1)
      ),
      avgMoodGain: parseFloat(
        (totalMood / safeCount).toFixed(1)
      ),
      avgEnergyGain: parseFloat(
        (totalEnergy / safeCount).toFixed(1)
      ),
      bestDecision,
      categoryBreakdown,
    });
    ...
    const timeline = checkins.map((c, i) => {
      ...
      const day =
        i === 0
          ? 0
          : Math.floor(
              (new Date(c.loggedAt) - new Date(baseline.loggedAt)) /
                msPerDay
            );
